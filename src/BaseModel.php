...
abstract class BaseModel extends Model
{
    ...
    public function getClient()
    {
        if (is_null($this->_client)) {
            if (empty($this->client_id)) {
                $this->errorServer(Yii::t('oauth2', 'Unknown client.'), Exception::INVALID_CLIENT);
            }
    ...
    public function validateClientSecret($attribute)
    {
        if (!Yii::$app->security->compareString($this->getClient()->client_secret, $this->$attribute)) {
            $this->addError($attribute, Yii::t('oauth2', 'The client credentials are invalid.'), Exception::UNAUTHORIZED_CLIENT);
        }
    }
    ...
    public function validateRedirectUri($attribute)
    {
        if (!empty($this->$attribute)) {
            ...
            if (strncasecmp($this->$attribute, $clientRedirectUri, strlen($clientRedirectUri)) !== 0) {
                $this->errorServer(Yii::t('oauth2', 'The redirect URI provided is missing or does not match.'), Exception::REDIRECT_URI_MISMATCH);
            }
        }
    }
    ...
    public function validateScope($attribute)
    {
        if (!$this->checkSets($this->$attribute, $this->client->scope)) {
            $this->errorRedirect(Yii::t('oauth2', 'The requested scope is invalid, unknown, or malformed.'), Exception::INVALID_SCOPE);
        }
    }
